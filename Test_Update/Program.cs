...
namespace Test_Update {
    class Program {
        ...
        List<Data_Insert> dt_Insert = new List<Data_Insert>();
        List<Data_Update> dt_Update = new List<Data_Update>();
        List<Columna_Informacion> columnas = new List<Columna_Informacion>();
        SqlConnection cnn = new SqlConnection(@"Data Source=LAPTOP-44FRKSCR\SQLEXPRESS; Initial Catalog =Rows-Testing; Integrated Security = True");
        static void Main(string [] args) {
            Program cs = new Program();
            cs.getRowInsert("dbo.STUDENTS4", 3);
            cs.getRowsUpdate("dbo.STUDENTS4", 3);
            cs.getColumns();
            //cs.printHash();

            //cs.getColumns();
            //cs.PrintColumns();
            //cs.getRowLogContents1("0000:000004ea");
            //cs.setValues();
        }
        public void getInsertsContent(string transaccion_id) {
        ...
        public void getRowsUpdate(string nombre_tabla, int slot_Id) {
            string sql = @"SELECT a.[RowLog Contents 0], a.[RowLog Contents 1],a.[Offset in Row],a.[Modify Size],a.[Transaction ID],b.[Begin Time] as Hora from fn_dblog(null,null) a, (SELECT  * from fn_dblog(null,null) b  where b.operation= 'LOP_BEGIN_XACT') b
                        WHERE b.[Transaction ID] = a.[Transaction ID] and a.[Slot ID]='" + slot_Id + "' and a.AllocUnitName = '" + nombre_tabla + "' and (a.operation = 'LOP_MODIFY_ROW'  ) order by b.[Begin Time] asc";
            SqlCommand command;
        ...
        public void getRowInsert(string nombre_tabla, int slot_Id) {
            string sql = @"SELECT [RowLog Contents 0] from fn_dblog(null,null) where operation = 'LOP_INSERT_ROWS' and [Slot ID]='" + slot_Id + "' AND AllocUnitName = '" + nombre_tabla + "'";
            SqlCommand command;
            SqlDataReader dataReader;
            try {
                cnn.Open();
        ...
        public void getColumns() {
            string sql = @"SELECT c.name AS column_name, max_inrow_length,pc.system_type_id, leaf_offset FROM sys.system_internals_partition_columns pc JOIN sys.partitions p ON p.partition_id = pc.partition_id JOIN sys.columns c ON column_id = partition_column_id
                         ...
                         WHERE p.object_id=object_id('dbo.STUDENTS4')  order by leaf_offset asc;";
            SqlCommand command;
            SqlDataReader dataReader;
            try {
                cnn.Open();
...
//    List<byte> canValues = new List<byte>();
//    for (int i = cu.offset; i < rowLog[0].rowLogContent.Length; i++) {

//        if (cu.rowLogContent[i].ToString() != "0" && cu.rowLogContent[i+1].ToString() !="0") {
//            Console.WriteLine("Limite alcanzado");
//        } else if(cu.rowLogContent[i].ToString() == "0") {
//            Console.WriteLine("Saltando Valor");

//        } else {
//            Console.WriteLine(cu.rowLogContent [i]);
//            canValues.Add(cu.rowLogContent [i]);
//        }
//        i++;
//    }
//    int canTamanoVar = columnas.Where(x => x.offset < 1).ToList().Count;
//    int valor = canTamanoVar- canValues.Count ;
//    if ((canTamanoVar - valor)==1) {
//        byte [] valores = cu.rowLogContent;
//        int nuevoTam = cu.rowLogContent.Length - cu.hex_Old.Length;
//        Array.Resize<byte>(ref valores,nuevoTam + cu.hex_New.Length +2);
//        Array.Copy(cu.hex_New, 0, valores, cu.offset, cu.hex_New.Length);
//        Console.WriteLine("Puede reemplazar todos los bytes restantes!");
//    } else {
//        Console.WriteLine("Debes hacer desplazaminto");
//    }
...
//        Console.WriteLine("nombre: "+ columnas[i].column_name);
//        Console.WriteLine("offset: "+ columnas[i].offset);
//        Console.WriteLine("capacidad: "+ columnas[i].capacity +"\n");
//    }
//}
...
//                WHERE b.[Slot ID] = a.[Slot ID] and a.AllocUnitName = 'dbo.STUDENTS4' and a.operation = 'LOP_MODIFY_ROW' and a.[Transaction ID] ='" + id_transaction + "'";
//    SqlCommand command;
//    SqlDataReader dataReader;
//    try {
//        cnn.Open();
